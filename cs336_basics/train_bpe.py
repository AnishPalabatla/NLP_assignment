...
# Each worker process compiles its own regex instance
RX=None
...
def build_word_freq_parallel(
    ...
    *,
    ...
    Uses many small chunks+imap_unordered so fast workers immediately pick up
    more work instead of sitting idle while a slow worker finishes a big chunk.
    """
    ...
    merged=Counter()
    print(f"[train_bpe] dispatching {len(tasks)} chunks across {num_processes} workers...", flush=True)
    done=0
    with Pool(processes=num_processes, initializer=init_worker, maxtasksperchild=8) as pool:
        for partial in pool.imap_unordered(process_chunk, tasks, chunksize=1):
            ...
            if done%32==0 or done==len(tasks):
                print(f"[train_bpe] pre-tokenized {done}/{len(tasks)} chunks", flush=True)
...
def train_bpe(
    ...
    *,
    ...
    # ---- pre-tokenization+counting (parallelized, load-balanced) ----
    if num_processes is None:
        num_processes=min(8, os.cpu_count() or 1)
    print(f"[train_bpe] cpu_count={os.cpu_count()}, using {num_processes} processes", flush=True)

    file_size=os.path.getsize(input_path)

    t0=time.time()
    if num_processes<=1 or file_size<1_000_000:  # ~1MB: serial is faster than pool startup
    ...
    print(f"[train_bpe] pre-tokenization done in {time.time()-t0:.1f}s, {len(word_freq)} unique pretokens", flush=True)
    ...
    while next_id<vocab_size:
        ...
        if not affected:
            ...
            # replace occurrences of (a,b) in every affected word
        ...
        if merge_num%200==0 or merge_num==target_merges:
            print(
                ...
                f"vocab_size={len(vocab)}, elapsed={time.time()-t0:.1f}s",
                flush=True
            )
